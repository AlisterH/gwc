...
pkgdatadir = $(datadir)/gtk-wave-cleaner
pkgincludedir = $(includedir)/gtk-wave-cleaner
pkglibdir = $(libdir)/gtk-wave-cleaner
pkglibexecdir = $(libexecdir)/gtk-wave-cleaner
...
am__DIST_COMMON = $(srcdir)/Makefile.in COPYING INSTALL README TODO \
...
#HELPDIR = $(DOCDIR)
DEFS = -DDATADIR=\"$(DATADIR)\" -DHELPDIR=\"$(HELPDIR)\" -DLIBDIR=\"$(LIBDIR)\" -DAPPNAME=\"$(APPNAME)\" -DMAC_OS_X   -DHAVE_FFTW3 -I/opt/homebrew/Cellar/fftw/3.3.10_2/include -DFFTWPREC=2  
DEPDIR = .deps
ECHO_C = \c
ECHO_N = 
ECHO_T = 
...
OGGLIB = 
OSXDEF = -DMAC_OS_X
OSXLIBS = -lm  -framework CoreAudio
...
meschach/meschach.a :
# Build Meschach without sparse matrix support (part3) since GWC doesn't use it
# and sparse.c has compilation issues with modern compilers
	(cd meschach ; \
	 ./configure ; \
	 sed -i.bak 's|/\* #undef STDC_HEADERS \*/|#define STDC_HEADERS 1|' machine.h ; \
	 sed -i.bak 's|/\* #undef HAVE_PROTOTYPES \*/|#define HAVE_PROTOTYPES 1|' machine.h ; \
	 sed -i.bak 's|/\* #undef HAVE_PROTOTYPES_IN_STRUCT \*/|#define HAVE_PROTOTYPES_IN_STRUCT 1|' machine.h ; \
	 make part1 ; make part2 ; cp machine.h ..)
