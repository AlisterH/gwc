...
/* Counters for UI position/progress */
 ...
 * Updates play_cursor + rendered_file_frames_abs.
 * (Metering removed: handled in gwc.c from sample-block summaries)
 * ---------------------- */
static void
render_interleaved_stereo_float(MacOSXAudioData *ad, float *out, UInt32 out_frames)
{
    ...
    while (frames_left > 0) {
        ...
        if (file_ch == 2) {
            /* Read directly into interleaved stereo out buffer */
            ...
            if (mono_tmp_frames_cap < (size_t)frames_to_read) {
                ...
                if (!nb) {
                    /* OOM: output silence for this chunk */
                    memset(out + (size_t)out_off * ch, 0,
...
macosx_audio_out_callback(AudioDeviceID device,
                         ...
                         void *client_data)
{
    (void)device; (void)current_time; (void)data_in; (void)time_in; (void)time_out;
...
/* ----------------------
 * Minimal process_audio (meters removed)
 * ---------------------- */
...
void audio_device_close(int drain)
{
    ...
    /* Item (5): free mono temp buffer */
    if (mono_tmp) {
        free(mono_tmp);
        mono_tmp = NULL;
        mono_tmp_frames_cap = 0;
    }
}